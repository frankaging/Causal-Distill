...
def load_teacher_setups(vocab_file, bert_config_file,
                        init_checkpoint,
                        label_list, 
                        num_train_steps,
                        do_lower_case=True, 
    ...
    if init_checkpoint is None:
        err_msg = "Error: Teachers model have to be based on a pretrained model"
...
def data_and_model_loader(device, n_gpu, args):
    ...
    if args.model_type == "TeacherBERT":
        model, tokenizer, optimizer = \
            load_teacher_setups(args.vocab_file, args.teacher_config_file,
...
def system_setups(args):
    ...
    if args.student_config_file is not None:
        student_config = BertConfig.from_json_file(args.student_config_file)
        if args.max_seq_length > student_config.max_position_embeddings:
            raise ValueError(
                "Cannot use sequence length {} because the teacher BERT model was only trained up to sequence length {}".format(
                args.max_seq_length, student_config.max_position_embeddings))

    if args.teacher_config_file is not None:
        student_config = BertConfig.from_json_file(args.teacher_config_file)
        if args.max_seq_length > student_config.max_position_embeddings:
            raise ValueError(
                "Cannot use sequence length {} because the teacher BERT model was only trained up to sequence length {}".format(
                args.max_seq_length, student_config.max_position_embeddings))
